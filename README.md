# BigOrange Marketing

## Cinematic editorial homepage

Static site concept packaged from the locally available Kimi design corpus for review.

- GitHub: `https://github.com/dillonmohr8777/bigorange-marketing-homepage`
- Live preview: https://bigorange-marketing-homepage.netlify.app
- Preview deployment: pushes to `main` rebuild the live preview automatically.
See [KIMI_PUBLISHING.md](KIMI_PUBLISHING.md) for the standing push and preview contract.

The site is a self-contained static build. Open `index.html` locally or serve this directory with any static web host.

The included dashboard data is aggregate and sanitized design-study content. No credentials, form submissions, cookies, or private account data are included.
